...
//    self.controllerToolBar.backgroundColor = COLOR_BACKGROUD_GRAY;
    self.controllerToolBar.backgroundColor = RGBColor(246, 247, 248, 1.0);
    ...
    self.finishBtn.titleLabel.font = [UIFont systemFontOfSize:18];
    [self.finishBtn addTarget:self action:@selector(finishBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    [self.finishBtn setTitle:NSLocalizedString(@"Save", nil) forState:UIControlStateNormal];
//    [self.finishBtn setTitleColor:self.tintColor forState:UIControlStateNormal];
    [self.finishBtn setTitleColor:RGBColor(0, 100, 246, 1.0) forState:UIControlStateNormal];
...
//    [self.cancelBtn setTitleColor:self.tintColor forState:UIControlStateNormal];
    [self.cancelBtn setTitleColor:RGBColor(0, 100, 246, 1.0) forState:UIControlStateNormal];
...
- (void)layoutSelfSubviews
{
    ...
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, SCREEN_WIDTH, 50);
    
    //完成按钮
    self.finishBtn.frame = CGRectMake(SCREEN_WIDTH - 100, 10, 90, 30);
    
    //取消按钮
    self.cancelBtn.frame = CGRectMake(10, 10, 90, 30);
    
    //标题
    self.titleLabel.frame = CGRectMake(70, 10, SCREEN_WIDTH - 140, 30);
}
